...
function responSrv(value) {
    fs.readFile(path1, 'utf8', (err, data) => {
        ...
        jsonData.respon = parseInt(jsonData.respon, 10) + parseInt(value, 10);

        fs.writeFile(path, JSON.stringify(jsonData, null, 2), (err) => {
            if (err) {
                console.error('Error writing file:', err);
            } else {
                console.log('Updated respon:', jsonData.respon);
            }
        });
    });
}

function readServerJson(path1, callback) {
    fs.readFile(path1, 'utf8', (err, data) => {
        if (err) {
            console.error('Error reading file:', err);
            callback(err, null);
...
app.get('/total-request', (req, res) => {
	readServerJson(path1, (err, jsonData) => {
    if (err) {
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>RoffiNeru4k - Total Request</title>
    <style>
        body {
            background-image: url('https://cdn.donmai.us/original/d7/aa/__eula_genshin_impact_drawn_by_bshi_edayo__d7aaebd87946789037a0ebb6cc59d5a9.jpg');
            background-size: cover;
...
<body>
    ...
    <script>
        function main() {
            const responsePre = document.getElementById("response");
            const responseObject = ${JSON.stringify(jsonData, null, 2)};
            responsePre.innerText = JSON.stringify(responseObject, null, 2);
        }
        main();
    </script>
...
app.get('/shutdown-exec-neru-fr-no-fek-fek', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
      ...
      <style>
        body, html {
          ...
          align-items: center;
          background: url('https://cdn.donmai.us/original/d7/aa/__eula_genshin_impact_drawn_by_bshi_edayo__d7aaebd87946789037a0ebb6cc59d5a9.jpg') no-repeat center center fixed;
          background-size: cover;
        }
        .container {
          width: 90%;
    ...
    <body>
      <div class="container">
        <p>Matikan Server</p>
        <button onclick="shutdownServer()">Matikan Server</button>
      </div>
      <script>
        function shutdownServer() {
          fetch('/shutdown-exec-neru-fr-no-fek-fek-memek-shutdown', { method: 'POST' })
            .then(response => {
              if (response.ok) {
...
app.post('/shutdown-exec-neru-fr-no-fek-fek-memek-shutdown', (req, res) => {
